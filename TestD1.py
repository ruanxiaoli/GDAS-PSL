...
if __name__ == '__main__':
    log_file = open("./train_final_noFESM_95_attention.log", "a")
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    net = MSTLoc_attention()
    # net.load_state_dict(torch.load("./model_save/model_D1_B4_M95_R7.pth"))
    weight = class_weight("./utils/D1data/train_D1.csv")
    ...
    train_set = Dataset("./utils/D1data/train_D1.npy", transform=None)
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4)
    test_set = Dataset("./utils/D1data/test_D1.npy")
    ...
    for epoch_index in range(epoch):
        ...
        if example_acc > best_acc:
            # Save model
            torch.save(net.state_dict(), "./model_save/model_D1_B4_M95_R7.pth")
