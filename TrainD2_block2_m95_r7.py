...
from model.MSTLoc_attention_95_2 import MSTLoc_attention
import ast
...
# Original weighted binary cross entropy loss function
class WeightedBinaryCrossEntropy(nn.Module):
    ...
    def forward(self, y_pred, y_true):
        ...
        #        combined_weights = [1,1,1,1,1,1]
        #        combined_weights = torch.tensor(combined_weights)
        # Initialize loss
...
# Add multi-label Focal Loss implementation
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, target):
        ...
        loss = - self.alpha * (1 - pt) ** self.gamma * target * torch.log(pt + epsilon) - (
                    1 - self.alpha) * pt ** self.gamma * (1 - target) * torch.log(1 - pt + epsilon)
...
if __name__ == '__main__':
    log_file = open("./train_final_D2_FLSD53_95_attention_1.log", "a")
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    net = MSTLoc_attention()
#    net.load_state_dict(torch.load("./model_save/model_D2_B4_M95_R7_1.pth"))
    weight = class_weight("./utils/D2data/train_D2_fv1.csv")
    # Focal Loss
    ...
    train_set = Dataset("./utils/D2data/train_D2_fv1.npy", transform=None)
    train_loader = DataLoader(train_set, batch_size=batch_size, shuffle=True, num_workers=4)
    test_set = Dataset("./utils/D2data/test_D2_fv1.npy")
...
#        exit()
        if example_acc > best_acc:
            # Save model
            torch.save(net.state_dict(), "./model_save/model_D2_B4_M95_R7_s_1.pth")
