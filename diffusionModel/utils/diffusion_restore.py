...
class Diffusion(object):
    ...
    # 用于从给定的张量中提取特定时间步的值。（可能把多个步长的图像拼接起来了，然后用时间步长获取）
    ...
    # 用于进行前向扩散过程，即在给定的图像上添加噪声。（会对输入的时间步长t进行一次深拷贝，然后在每个时间步长上随机选择一个值，范围是从0到当前时间步长。然后，使用这个随机选择的时间步长进行前向扩散。）
    # RSCD使用的方法
    ...
    def p_sample(self, model, x, t, t_index):
        ...
        # Use our model_MSTLoc (noise predictor) to predict the mean
        mout = model(x,t)
    ...
    # 方法用于在多个时间步上进行后向扩散过程。
    @torch.no_grad()
    # def p_sample_loop(self, model_MSTLoc, shape):
    ...
    # 用于生成指定数量的样本。
    @torch.no_grad()
    # def sample(self, model_MSTLoc, batch_size=16, channels=3):
