...
def extract_enhanced(region):
    enhanced = region[region.Reliability.apply(lambda x: x.replace(' ', '')).isin(['Enhanced'])]
...
def extract_supported(region):
    ...
    supported = region[region.Reliability.isin(['Supported'])]
    supported = supported.copy()
    # supported['label'] = supported[['Enhanced', 'Supported']].apply(
    #             lambda x: ';'.join(x), axis=1)
    supported['label'] = supported[['Supported']].apply(
                lambda x: ';'.join(x), axis=1)
...
def extract_approved(region):
    ...
    approved['label'] = approved[['Approved']].apply(
            lambda x: ';'.join(x), axis=1)
...
def extract_uncertain(region):
    ...
    uncertain['label'] = uncertain[['Uncertain']].apply(
                                        lambda x: ';'.join(x), axis=1)
...
if __name__ == "__main__":
    src = os.path.join(os.path.pardir, "./genelist",
